...
public class ShippingController {
    ...
    @PostMapping("/add")
    public ResponseEntity addShipping(@RequestBody @Valid Shipping shipping) {
        shippingService.addShipping(shipping);
        return ResponseEntity.status(200).body("Shipping added");
    }

    @PutMapping("/update/{id}")
    public ResponseEntity updateShipping(@PathVariable long id, @RequestBody @Valid Shipping shipping) {
    ...
    public ResponseEntity getShippingByStatus(@PathVariable String status) {
        List<Shipping> shippings = shippingService.getShippingByStatus(Shipping.ShippingStatus.valueOf(status.toUpperCase()));
        return ResponseEntity.ok(shippings);
    }
