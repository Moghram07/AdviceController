...
public class OrderItemController {
    ...
    @PostMapping("/add")
    public ResponseEntity addOrderItem(@RequestBody @Valid OrderItem orderItem) {
        orderItemService.addOrderItem(orderItem);
        return ResponseEntity.status(200).body("Order item added");
    }

    @PutMapping("/update/{id}")
    public ResponseEntity updateOrderItem(@PathVariable long id, @RequestBody @Valid OrderItem orderItem) {
